...
class SearchCore
{
	...
	public static function getIdsProducts($db, $id_product, $id_lang)
	{
		$tags = '';
		$tagsArray = $db->ExecuteS('
	...
	public static function indexation($full = false, $id_product = false)
	{
		...
		$weightArray = array(
			...
			'product' => Configuration::get('PS_SEARCH_WEIGHT_REF'),//MBJ
		...
		while ($products = Search::getProductsToIndex($nbLanguages, 50) AND count($products) > 0)
		{
			...
			foreach ($products as $product)
			{
				...
				$product['product'] = Search::getIdsProducts($db, (int)$product['id_product'], (int)$product['id_lang']);//MBJ
